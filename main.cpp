/*
    ...
    Purpose:Project Two - Blackjack
            V12 - Final version. Incorporating operator overloading as final
                  needed checklist component. Cleaning/finalizing code and
                  adding any needed or additional comments.
 */
...
#include <algorithm>   //Algorithm Library STL
...
#include "BetCnt.h"
#include "LrgBet.h"
#include "HouseMoney.h"
#include "BestHndPoss.h"
...
int main(int argc, char** argv) {
    ...
    unsigned char nDeal = 1;             //# of cards to deal
    ...
    //Dealer is treated like a player but has other abilities (plyr[4])
    ...
    //For loop takes in bets and print
    for(int i = 0; i < 4; i++){
        cout << "Player " << i+1 << ": \n";
        bet = pBet.pBets();
    ...
    //Count of 4 players making a bet using static member bCount of BetCnt class
    bet1.bCntTtl();
    cout << endl;
    
    //Operator + overload, best hand possible for blackjack is 21
    //Display before hands are delt
    BestHndPoss doub1;
    BestHndPoss doub2;
    BestHndPoss doub3 = doub1 + doub2;
    cout << setprecision(0);
    cout << "\nThe best Hand Possible in BlackJack is " 
         << doub3.x << endl << endl;
    ...
    //Print the result of the game using ptResult() function
    ptReslt(handTtl, bets);
    
    //Display the least to largest hand played in the game using sort STL
    sort(handTtl, handTtl + 4);
    cout << "The least to greatest hands used in the game - \n";
    for(int i = 0; i < 4; i++){
        cout << handTtl[i] << endl;
    }
    
    //Display the largest bet, just for fun, no real meaning to the game
    //Getting a hang of/use of templates
    int oneBet, //Bet of first player
        twoBet, //Bet of second player
        thrBet, //Bet of third player 
        frBet;  //Bet of fourth player
    
    //Initialize each bet integer to the bets array for each player.
    oneBet = bets[0];
    twoBet = bets[1]; 
    thrBet = bets[2]; 
    frBet = bets[3];
    int big1; //Biggest bet between players 1 and 2
    int big2; //Biggest bet between players 3 and 4
    //If else statements until two largest bets are calculated
    if(oneBet > twoBet){
        big1 = oneBet;
    }
    else{
    ...
    //LrgBet template class instance
    LrgBet <int> bigBet(big1, big2);
    //Display
    cout << fixed << setprecision(2) << showpoint;
    cout << "\nThe largest bet(or winnings/losings if a player won/lost) "
         << "made in today's "
         << "game was $" 
         << bigBet.bigger();
